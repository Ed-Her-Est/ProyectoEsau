...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $csvFileName = 'backup_Cliente_' . date('Y-m-d_H-i-s') . '.csv';

    // Encabezados del archivo CSV
    $csvData = "ClienteID,nombre,apellidoPaterno,apellidoMaterno,created_at,updated_at,deleted_at\n";
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        $csvData .= "{$row['ClienteID']},{$row['nombre']},{$row['apellidoPaterno']},{$row['apellidoMaterno']},{$row['created_at']},{$row['updated_at']},{$row['deleted_at']}\n";
    }
...
<body>
    <div class="container mt-5">
        ...
        <div class="container mt-5">
            <a class="btn btn-success" href="mostrarCliente.php">Regresar</a>
        </div>
    </div>
